...
│ (BM25 Search &    │     │  (str_replace,    │     │  (chunk+embed,   │  │  (docker exec,   │
...
- **Workspace Search** — Two distinct modes, not one feature with a toggle:
  ...
  `semantic=true` switches to BM25 keyword ranking with a code-aware tokenizer
  (splits `camelCase`, `snake_case`, `PascalCase`) — but that mode only covers
  tracked files in `scratch/`; it doesn't see `uploads/`. Note this "semantic"
  is BM25 concept-level matching, not embeddings — actual embedding-based
  semantic search is reserved for `doc_search` and `search_history` (see
...
|_______________________|_________________________________________|______________________________|

Only NVIDIA is required for the full feature set (embeddings, reranking, OCR). Linkup is
needed for web search. Each cloud LLM backend is optional — the agent starts fine
without any of them configured. Note that a missing key isn't always surfaced up
front: `WebAgent` and `ImageOCRAgent` print a warning at startup if their key is
missing, but a missing Google/NIM/Cloudflare key currently fails silently until you
actually try to use that backend, at which point the API call errors out. Configure
the key for whichever backend you plan to use as your default before you start
chatting with it.
...
|_______________________| (Global History permanent chat storage still survives)______|
...
<!-- BM25 concept search across scratch only -->
<workspace_search>
...
- **XML Tool Calling as Primary Format**: Tool calls are emitted as XML tags inside
  ...
  When running on Kimi via NIM, the agent also parses Kimi's native
  `<|tool_call_begin|>` format and deduplicates against the XML tags, so neither format
  is missed.

- **BM25 Over Embeddings for Code Search**: Code search runs many times per turn and
  needs to feel instant, so `workspace_search`'s BM25 mode trades semantic depth for
  \~5ms, zero-API-call lookups (see Workspace Search above for how the two modes and
  their scopes differ). Embeddings are reserved for PDFs and global history, where
  the payoff (finding conceptually related passages, not just matching tokens) is
  worth the extra latency and API cost.
